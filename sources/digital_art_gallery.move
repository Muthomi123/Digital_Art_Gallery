module digital_art_gallery::digital_art_gallery {
    ...
    public entry fun create_artwork(
        ...
        assert!(coin::value(&payment) >= price, INSUFFICIENT_FUNDS);
    ...
    // Function to change the artist of a Gallery
    public entry fun change_gallery_artist(
        gallery: &mut Gallery,
        new_artist: address,
        ctx: &mut TxContext
    ) {
        assert!(gallery.artist == tx_context::sender(ctx), NOT_THE_OWNER);
        gallery.artist = new_artist;
    }
    #[test_only]
